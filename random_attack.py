...
class RandomAttack:
    def __init__(
        ...
        # NOTE: language_tool_python.LanguageTool spins up a local Java server and
        # can be quite heavy / slow to start. For our character-level attacks
        # that only use simple homoglyph replacement (char_op=2) we do not need
        # a spellchecker at all, so we leave this as None by default. Code paths
        # that rely on it (e.g. char_op=6 with OCR-style corrections) will
        # gracefully fall back to using the raw word.
        self.spellchecker = None
        if isinstance(tokenizer,str) and len(tokenizer)>0:
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer)
        else:
            self.tokenizer = tokenizer
        self.vocab_size=self.tokenizer.vocab_size
        try:
            if isinstance(ref_model, str) and len(ref_model)>0:
    ...
    def ref_score(self, sentences, target_class):
        ...
        if logits.ndim == 2 and logits.size(1) > target_class:
            fitness = logits[:, target_class]
        else:
            fitness = logits.reshape(-1)
                    ...
                    if operation == 1:  # Delete
                        ...
                        for idx, word in enumerate(tmp_token_list):
                            ...
                            if 'ocr' in self.def_stl:
                                ...
                                # If no external spellchecker is configured, just
                                # use the raw candidate word as-is.
