...
# ------------------------------------------------------------
# Fixed defaults for "fairness / alignment"
# ------------------------------------------------------------
USE_PREFIX_BYTES_IN_PRF: bool = True              # 强制：PRF 使用 token 内 prefix-bytes（pos内）
BYTEKGW_ALL_MAX_BYTE_POS: int = 64                # 全字节偏置/检测
BYTEKGW_SCHEME: str = "byte_tree"                 # 你当前在用 byte_tree
CAP_CONT_TOKENS_FOR_EVAL: bool = True             # 检测+PPL 只用前 K 个 continuation token
PROMPT_WORDS: int = 20                            # C4 prompt 取前 20 词
...
def worker_main(rank: int, device_str: str, items: List[PromptItem], args_dict: Dict[str, Any], q: mp.Queue):
    ...
    # ------------------------------
    # HF baseline (optional but useful)
    # ------------------------------
    if bool(args_dict["emit_hf_baseline"]):
        for idx, item in enumerate(items):
            ...
            if (idx + 1) % max(1, len(items) // 2) == 0 or (idx + 1) == len(items):
                print(f"[GPU {device_str}] HF baseline ({idx+1}/{len(items)})", flush=True)

    # ------------------------------
    # ByteKGWv5(all-bytes) generations for deltas
    # ------------------------------
    for delta in deltas:
        # ByteKGWv5 all-bytes temp cfg
        tmp_b = make_temp_cfg(bytekgw_config, {
            "delta": float(delta),
            "scheme": BYTEKGW_SCHEME,
            "max_byte_pos": int(BYTEKGW_ALL_MAX_BYTE_POS),
            "use_prefix_bytes_in_prf": True,
            "add_special_tokens": bool(add_special_tokens),
            "use_torch_generator": False,

            # 兼容旧字段：如果还存在 detector_mode
            "detector_mode": "all_bytes",

            # 强制 gen 参数（使 ByteKGWv5 内部与 HF 一致）
            "gen": dict(gen_kwargs),
        })
        ...
        try:
            ...
            for idx, item in enumerate(items):
                # 为了不同 delta 之间可复现且互不冲突：seed 加 delta 偏移
                seed = seed_base + item.prompt_id + int(delta) * 100000
                ...
                det_b = bytekgw.detect_watermark(trunc_full, return_dict=True, add_special_tokens=False)
                stat_b = extract_stat(det_b)

                ppl_b, cont_len_b = ppl_continuation_from_ids(model, trunc_ids, prompt_end, device=device)

                add_row(
                    f"bytekgw_all_delta{int(delta)}",
                    row_base(
                        algo="ByteKGWv5(all)",
                        delta=float(delta),
                        device=device_str,
                ...
                if (idx + 1) % max(1, len(items) // 2) == 0 or (idx + 1) == len(items):
                    print(f"[GPU {device_str}] delta={int(delta)} ({idx+1}/{len(items)})", flush=True)
...
def main():
    ...
    ap.add_argument("--c4_paths", type=str,
                    default="dataset/c4/realnewslike/c4-train.00000-of-00512.json.gz",
    ...
    ap.add_argument("--emit_hf_baseline", action="store_true",
                    help="Also write hf_generate.csv for the same prompts (recommended).")

    ap.add_argument("--out_dir", type=str, default="outputs/c4_samples_bytekgw_all_200")
    ...
    with open(meta_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "emit_hf_baseline": bool(args.emit_hf_baseline),
            "gen": {
            ...
            "bytekgw_all_max_byte_pos": BYTEKGW_ALL_MAX_BYTE_POS,
    ...
    print(f"[ByteKGWv5(all)] scheme={BYTEKGW_SCHEME} max_byte_pos={BYTEKGW_ALL_MAX_BYTE_POS} use_prefix_bytes_in_prf={USE_PREFIX_BYTES_IN_PRF}")
    ...
    # write CSVs
    if args.emit_hf_baseline:
        write_csv(
            os.path.join(args.out_dir, "hf_generate.csv"),
            sorted(merged.get("hf_generate", []), key=lambda r: r["prompt_id"])
        )

    for d in deltas:
        dk = int(d)
        write_csv(
            os.path.join(args.out_dir, f"bytekgw_all_delta{dk}.csv"),
            sorted(merged.get(f"bytekgw_all_delta{dk}", []), key=lambda r: r["prompt_id"])
        )
    ...
    for d in deltas:
        dk = int(d)
        print(f"  - {os.path.join(args.out_dir, f'bytekgw_all_delta{dk}.csv')}")
    print(f"  - {meta_path}")
