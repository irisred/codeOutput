...
# -*- coding: utf-8 -*-
"""
Multiprocess DiP generator: splits prompts across workers/devices, writes per-strength CSVs.

Example:
TOKENIZERS_PARALLELISM=false /home/star/jf/python/stega/bin/python scripts/generate_clean_dip_mp.py \
  --run_meta outputs/c4_samples_head_200/run_metadata.json \
  --dip_config config/DIP.json \
  --output_dir outputs/dip_gen_mp \
  --n_prompts 200 \
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--alphas", default=None, help="comma-separated alpha overrides; None=use config")
    ap.add_argument("--gammas", default=None, help="comma-separated gamma overrides; None=use config")
...
def worker(task: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, List[Dict[str, Any]]]]:
    devs = task["devices"]
    ...
    dip_cfg_path = task["dip_cfg_path"]
    seed_base = task["seed_base"]
    alpha_list = task["alpha_list"]
    gamma_list = task["gamma_list"]
    ...
    for prompt_id, prompt_row in iterable:
        ...
        for alpha in alpha_list:
            for gamma in gamma_list:
                ...
                out_dip = mdl.generate(**enc, logits_processor=LogitsProcessorList([dip_proc]), **gen_params)[0]
                full_dip, cont_dip = split_prompt_cont(tok, out_dip, prompt_text, add_special_tokens)
                key = f"a{alpha if alpha is not None else dip_cfg_obj.alpha}_g{gamma if gamma is not None else dip_cfg_obj.gamma}"
                rows = outputs.setdefault(key, [])
                rows.append(
                    {
                        "algo": "dip",
...
def main() -> None:
    ...
    outputs_all: Dict[str, List[Dict[str, Any]]] = {}
    ...
    if n_workers == 1:
        ...
        for k, v in out.items():
            outputs_all.setdefault(k, []).extend(v)
    else:
        tasks = []
    ...
    for key, rows in outputs_all.items():
        pd.DataFrame(rows).to_csv(out_dir / f"dip_{key}.csv", index=False)
    print(
        f"wrote clean={0 if args.skip_clean else len(rows_clean_all)} and "
        f"{sum(len(v) for v in outputs_all.values())} dip rows to {out_dir}"
    )
